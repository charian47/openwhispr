...
function DiarizationSection() {
  ...
  const checkModelStatus = useCallback(async () => {
    try {
      ...
      if (status) {
        setModelsDownloaded(status.modelsDownloaded);
      }
    } catch {
      // ignore
    } finally {
  ...
  useEffect(() => {
    const unsubscribe = window.electronAPI?.onDiarizationDownloadProgress?.((data: any) => {
      if (data.type === "complete") {
        setIsDownloading(false);
        setModelsDownloaded(true);
        setDownloadProgress(100);
      } else if (data.percentage !== undefined) {
        setDownloadProgress(Math.round(data.percentage));
      }
    });
    return () => {
      unsubscribe?.();
  ...
  const handleDelete = async () => {
    try {
      await window.electronAPI?.deleteDiarizationModels?.();
  ...
  return (
    <div>
      <SectionHeader
        title={t("settingsPage.transcription.diarization.title")}
        description={t("settingsPage.transcription.diarization.description")}
      />
      <SettingsPanel>
        {modelsDownloaded ? (
          <SettingsPanelRow>
            <SettingsRow label={t("settingsPage.transcription.diarization.modelsReady")}>
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1.5 text-xs text-success">
                  <Check size={14} />
                  <span>{t("settingsPage.transcription.diarization.modelsReady")}</span>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
          ...
          <SettingsPanelRow>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-foreground">
                  {t("settingsPage.transcription.diarization.downloading")}
                </span>
          ...
          <SettingsPanelRow>
            <SettingsRow
              label={t("settingsPage.transcription.diarization.toggle")}
              description={t("settingsPage.transcription.diarization.requiresModels")}
            >
              <Button
                variant="outline"
                size="sm"
              ...
              >
                <Download size={12} />
                {t("settingsPage.transcription.diarization.downloadModelsSize")}
              </Button>
            </SettingsRow>
          </SettingsPanelRow>
        )}
